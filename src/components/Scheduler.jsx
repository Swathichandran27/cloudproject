...
//         }))
//       };
//       /*const response = await fetch('https://po136ctdyf.execute-api.us-east-2.amazonaws.com/dev/task'*/
//       const response = await fetch('https://i90kvp9nl5.execute-api.us-east-2.amazonaws.com/prod/schedules', {
//         method: 'POST',
//         mode:'cors',
//         headers: {
        ...
        <>
          ...
          {scheduledDetails && (
            <div className="scheduled-details">
              ...
              {scheduledDetails.scheduleType === 'single' ? (
                <>
                  <p>Delivery Date: {new Date(scheduledDetails.deliveryInfo.deliveryDate).toLocaleDateString()}</p>
                  <p>Delivery Time: {new Date(scheduledDetails.deliveryInfo.deliveryTime).toLocaleTimeString()}</p>
                </>
              ) : (
                <>
                  <p>Recurring Days: {scheduledDetails.deliveryInfo.recurringDays.join(', ')}</p>
                  <p>Start Date: {new Date(scheduledDetails.deliveryInfo.startDate).toLocaleDateString()}</p>
                  <p>End Date: {scheduledDetails.deliveryInfo.endDate ? new Date(scheduledDetails.deliveryInfo.endDate).toLocaleDateString() : 'Not Set'}</p>
                  <p>Delivery Time: {new Date(scheduledDetails.deliveryInfo.recurringTime).toLocaleTimeString()}</p>
                </>
              )}
              <p>Delivery Address: {`${scheduledDetails.address.flatNo}, ${scheduledDetails.address.street}, ${scheduledDetails.address.city}, ${scheduledDetails.address.postalCode}`}</p>
